...
import_export_path <- file.path(data_dir, '01-G-import-export-local')
interconnector_power_path <- file.path(data_dir, '01-G-interconnector-power.parquet')
setgendataregion_temp_dir <- file.path(data_dir, '01f-SETGENDATAREGION')
dest_path <- file.path(data_dir, '01-G-aemo-joined-all.parquet')
...
# calculate some interesting stats for the report
# to start with, let's get some stats like
# "the top x% of time periods account for y% of total costs"
# Our analysis is not about costs, but these are the most skewed.
# This is in table SETGENDATAREGION
# https://nemweb.com.au/Reports/Current/MMSDataModelReport/Electricity/MMS%20Data%20Model%20Report_files/MMS_331.htm#1
# Note that this table was 30 minute granularity,
# then changed to 5 minute granularity
# This data is also big, and if_else doesn't work with arrow
# so we repartition it onto disk to split into
# before and after the granularity change.
...
SETGENDATAREGION <- open_dataset(file.path(source_dir, 'SETGENDATAREGION')) |>
  select(
    SETTLEMENTDATE, 
    PERIODID, 
    REGIONID, 
    ...
    PERIODID,
    VERSIONNO) |>
  arrange(desc(SCHEMA_VERSION), desc(VERSIONNO), desc(TOP_TIMESTAMP), desc(LASTCHANGED)) |>
  select(-SCHEMA_VERSION, -VERSIONNO, -TOP_TIMESTAMP, -LASTCHANGED) |>
  collect() |>
  distinct(SETTLEMENTDATE, PERIODID, REGIONID, .keep_all = TRUE) |>
  select(SETTLEMENTDATE, PERIODID, REGIONID, ENERGYCOST, NETENERGY) |>
  mutate(
    INTERVAL_DURATION = if_else(SETTLEMENTDATE >= interval_change_date, 
                                new_interval_length, 
                                old_interval_length),
    INTERVAL_END = SETTLEMENTDATE + minutes(INTERVAL_DURATION * PERIODID),
    INTERVAL_START = INTERVAL_END - minutes(INTERVAL_DURATION),
  )

# now generate the stats about cost
SETGENDATAREGION |>
