...
hourly <- hourly |> 
  mutate(
    co2_ratio = co2_per_capita / co2_midday_per_capita
  )
...
daily <- daily |>
  ...
  rename(
    ...
    co2_ratio_at_transition=co2_ratio,
  ) |>
  right_join(daily) |>
  mutate(
    ...
    co2_ratio_compared_to_transition=co2_ratio / co2_ratio_at_transition
  )
hourly <- hourly |>
  filter(
    Date == dst_date,
...
agg_d |>
  ggplot(aes(x=days_into_dst, co2_per_capita_compared_to_transition, color=treatment)) + 
  geom_smooth() + 
  labs(
...
agg_d2 |>
  ...
  ggplot(aes(x=days_into_dst, co2_per_capita_compared_to_transition, color=paste(treatment, dst_direction))) + 
  geom_smooth()
